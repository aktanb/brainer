module Brainer {
    export class InnerGame {
        public settings: any;
        public sessionId: string;
        public answers: Array<any>;
        public frames: Array<any>;
        public thinkingPerframeTimes: Array<number>;
        ...
        public get playScene(): any {
            return _site.playground.playScene;
        }

        constructor(public parent: MainGame, rawGameData: any) {
            var _this = this;
            _this.settings = JSON.parse(rawGameData.settings);
            _this.sessionId = UUID.generate();
            _this.answers = new Array<any>();
            _this.frames = new Array<any>();
            _this.frames = JSON.parse(rawGameData.frames);
            _this.thinkingPerframeTimes = new Array<number>();
        ...
        public initialize() {
            var _this = this,
                veryFirstFrame: boolean = _this.parent.currentInnerGameIndex === 0 && _this.currentFrameIndex === 0;

            _this._sequence = new Array<number>();

            _site.playground.playScene.find('.active-section, .wannabe-active-section')
                .hover(function () {
                    if (!$(this).hasClass('task-section')) { $(this).addClass('active-section-hovered'); }
                }, function () {
                    if (!$(this).hasClass('task-section')) { $(this).removeClass('active-section-hovered'); }
                })
                .click(function () {
                    ...
                    if (_this.playScene.hasClass('one-click-answer')) {
                        ...
                        if (_this.name === "find_the_corresponding_figure") {
                            var rightAnswers = _this.playScene.find('.task-section'),
                                selectedSections = _this.playScene.find('.section-selected');
                        ...
                        if (_this.name === "sequence" && _this._sequenceReady === true) {
                            ...
                            var activeSections = _this.playScene.find('.active-section'),
                                selectedSections = _this.playScene.find('.section-selected');
            ...
            if (_this.name === "sequence") {
                setTimeout(function () {
                    var confusedButton: any = $('.confused-button');
            ...
            setTimeout(function () {
                var objectsToCenterVertically: any = $('.task-section-container, .equals-sign, .playground-body');
        ...
        private nextFrame(answeredRight: boolean, skipped: boolean = false) {
            var _this = this;
        }
        ...
        private _sequence: Array<number>; // Массив для хранения нажатых чисел в игре про последовательность.
        private _sequenceReady: boolean = false; // Флаг, указывающий на то, что уже можно приступать к отгадыванию в игре про последовательность.
        private _backspaceSuppressed: boolean = false;
    }
}